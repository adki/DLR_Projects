...
def save_header_conv_bn( num_conv
    ...
    if bn is not None:
        ...
        fullprint_array_comma(rvar, file=fp)
        print("};", file=fp)
    else:
        if conv.bias is not None:
            if conv.bias.is_cuda:
...
def save_header_fc( num
    ...
    if fc.bias is not None:
        ...
        print(f"const float fc{num}_bias[{nelms}]=", "{ //", f" {fc.bias.data.shape}", file=fp)
    ...
    print(f"const float fc{num}_weight[{nelms}]=", "{ //", f" {fc.weight.data.shape}", file=fp)
    fullprint_array_comma(weights, file=fp)
    print("};", file=fp)
